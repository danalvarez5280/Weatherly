...
export default class App extends React.Component {
  ...
  setToLocal() {
    let locale = this.state.city;
    let topCities = JSON.parse(localStorage.getItem('recentCities')) || [];
    ...
    let currentCity = JSON.stringify(locale);
    let recentCities = JSON.stringify(topCities)
  ...
  componentWillMount() {
    let oldLocation = JSON.parse(localStorage.getItem('currentCity'))
    console.log('stored city', oldLocation)
    if (oldLocation) {
      this.setState({
        city: oldLocation
      }, this.submitClicked)
